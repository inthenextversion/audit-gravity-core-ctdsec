async function main() {
    ...
    if (chainId != 137){//Main net addresses
      ...
      let GFI = await GravityToken.deploy("Mock GFI", "mGFI");
      await GFI.deployed();
      console.log("mGFI deployed to: ", GFI.address);
      gfiAddress = GFI.address;
  }

    const Vesting = await ethers.getContractFactory("VestingV2");
    const vesting = await Vesting.deploy(gfiAddress, wethAddress, wethAddress, 1625122800, 2592000);//wethAddress temp subbing in for governance
